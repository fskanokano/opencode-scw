#!/usr/bin/env node
/**
 * Deno 路径黑盒测试（TDD 先行断言，spec §9）。
 *
 * 用法：node scripts/test-deno.mjs [PORT]
 *   - PORT 缺省时自动拉起 `deno run --allow-all deno/main.ts`（DENO_BIN 可覆盖 deno 路径）
 *   - 传入 PORT 则只跑断言（外部已起好服务）
 *
 * 断言清单（对照 src/bridge.js test-bridge 语义 + spec §5 单门制）：
 ...
 *   D5  POST /v1/chat/completions（stream:false）→ 200 OpenAI 结构 + reasoning_content/content
...
async function main() {
  ...
  if (!base) {
    ...
    console.log(`==> 拉起 ${denoBin} run --allow-all deno/main.ts（PORT=18432）`)
